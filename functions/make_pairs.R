...
make_pairs <- function(x) {
    ...
    # Create a frequency table that counts the number of times Item2 appears 
    item2_count <- data.frame(table(eHist$Item2))  
    item2_count$Var1 <- as.character(item2_count$Var1)  # convert to character from factor
    ...
    # combination occur
    item_pairs <- data.frame(table(eHist$Item1, eHist$Item2)) %>% 
        filter(Freq > 0) 
    
    # Order by Item1 (Var1) 
    item_pairs <- item_pairs %>% arrange(Var1)  
    
    # Add up all movements from Item1 (Var1) and save as a separate reference table
    ...
    # Bring in the count of each unique item and align it with the item pair based on ITEM1 column 
    ...
    item_pairs <- item_pairs %>%
      dplyr::arrange(Var1, desc(Freq))
    
    # Rename some variables for clarity
    item_pairs <- item_pairs %>%
      dplyr::rename(Item1 = Var1, Item2 = Var2, PairCt = Freq)
    
    # Calculate the probability of each pair in item_pairs
    item_pairs <- item_pairs %>%
      dplyr::mutate(Prob = round(PairCt / ItemCt, 5)) %>%
      dplyr::arrange(Item1, -Prob, -PairCt)  # decreasing order by item, probability, then by count of each pair
    ...
    item_pairs$Item1Name <- sapply(item_pairs$Item1, get_title)  # get title names
    
    item_pairs$Item2Name <- sapply(item_pairs$Item2, get_title)  # get title names
    
    # Find any NA values in title names and replace with most recent title from data (if any)
    item_pairs <- item_pairs %>%
      dplyr::mutate( Item1Name = ifelse( !is.na(Item1Name),
                                    Item1Name,
    ...
    # Bring in the number of current incumbents in job 2
    # item_pairs$Incumbents <- item2_count[match(item_pairs$Item2, item2_count$Var1),"Freq"]
    item_pairs$Incumbents <- incumbents[match(item_pairs$Item2, incumbents$TitleCode),"Count"]
    ...
    item_pairs <- item_pairs %>%
      dplyr::select(-Item1copy, -Item2copy)
    
    item_pairs$Hyperlink <- ifelse( grepl("\\(UC\\)|/UC|\\bUC\\b|\\bUNCLASSIFIED\\b|\\bUNCLASSSIFIED\\b", item_pairs$Item2Name),
                                    NA,
                                    ifelse( is.na(item_pairs$Salary2),
                                            NA,
                                            substr(item_pairs$Item2, 1, 4) ) )
    
    # TESTING - if no hyperlink, make the salary blank
    item_pairs$Salary2 <- ifelse( is.na(item_pairs$Hyperlink),
                                  NA,
                                  item_pairs$Salary2)
    
    item_pairs$Salary2Min <- ifelse( is.na(item_pairs$Hyperlink),
                                  NA,
                                  item_pairs$Salary2Min)
    
    # Find Salary Difference
    item_pairs <- item_pairs %>%
      dplyr::mutate(SalaryDiff = round(Salary2-Salary1, 2))   # Find difference in salaries
    
    # Order and select the final the variables 
    item_pairs <- item_pairs %>%
      dplyr::select(Item1, Item1Name, Item2, Item2Name, PairCt, ItemCt, Prob, 
               Incumbents, SalaryDiff, Salary1, Salary2, Salary1Min, Salary2Min, Hyperlink)
    
    # Take the top 20 for every job in Item1 column (ties allowed)
    item_pairs <- item_pairs %>%
      dplyr::group_by(Item1) %>%
      dplyr::top_n(n = 20, wt = Prob)
